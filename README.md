...
**拟真模式** — 认知拟人化对抗贝叶斯反检测：安装级人格向量（节奏/失误率/惯用开局）；最少猜测目标按候选池规模 + 历史均步自动推算（保守/均衡/激进预设）；log-normal 思考时间模型（反馈/冷门度耦合、连击后犹豫、AFK 停顿）；成簇困惑失误 + 概率性绿格回退 + 同国籍垫刀；放水聚簇分布
...
**多人** — 默认只填入不提交；面板点「自动提交」开启全自动。「拟真」开关展开后选策略预设（延迟区间与放水率可选调），猜测目标自动计算。
