...
**CS:GO/CS2 选手猜测游戏辅助油猴脚本 — 求解、填入、拟真、反检测**
...
**单人/多人接管** — 开局自动计算填入；多人 DOM 轮询解析反馈，自动检测回合/场次结束与重赛续接。候选集按服务器难度池同步：入门版 322、简单版 384、完整版 646（数据覆盖不足时安全回退全量池）

**拟真模式** — 安装级人格向量（节奏/失误率/恢复率/惯用开局）；单人和多人共用同一套反馈驱动决策状态；最少猜测目标按当前难度池规模 + 有效历史均步自动推算（保守/均衡/激进预设）；log-normal 思考时间模型使用典型范围和软尾部（反馈/冷门度耦合、连击后犹豫、AFK 停顿）；成簇困惑失误 + 概率性回退 + 同国籍垫刀；放水计划先尝试若干手，再按计划止损

**反馈分布 governor** — 跨局滚动平均 `entropyPercentile`，连续调整信息增益偏好带，避免固定阈值切换造成明显控制器指纹
...
**多人** — 默认只填入不提交；面板点「自动提交」开启全自动。「拟真」开关展开后选策略预设（典型延迟范围与止损率可调），面板会显示当前难度池、阶段、目标回合和止损计划。输入框中的延迟范围是典型范围，实际采样允许自然软尾部；`0~0s` 表示关闭额外延迟。

**面板** — 标题栏可拖拽，`─` 收起为浮标。「选手库」卡片展示来源构成（仓库/本地/服务器）与同步进度；支持键盘焦点操作、窄屏自适应和 `prefers-reduced-motion`。

## 离线回归模拟

仓库提供不访问线上服务的决策冒烟模拟器，用真实 `data/players_full.json` 检查三种难度池、胜负/跳过/耗尽分布、候选内回退和信息增益统计。使用固定种子可复现：

```powershell
cd friberg-helper
node scripts/simulate.mjs --rounds=90 --seed=20260806
node scripts/simulate.mjs --rounds=90 --seed=7 --strategy=conservative
node scripts/simulate.mjs --rounds=90 --seed=99 --strategy=aggressive
```

模拟器只验证本地决策核心，不连接游戏服务器，也不等价于线上分析服务。
...
├── scripts/simulate.mjs     # 固定种子离线决策模拟
├── README.md
└── LICENSE                  # MIT
```
